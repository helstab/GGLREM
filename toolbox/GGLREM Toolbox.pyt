...
class Update(object):
    ...
    def execute(self, parameters, messages):
        ...
        zone_field = 'OBJECTID'
        in_value_raster = parameters[1].valueAsText
        target = parameters[2].valueAsText
        arcpy.AddMessage(target)
        t = numpy.array(target, dtype=numpy.float32)
        t2 = t * -1
        arcpy.AddMessage(t)
        arcpy.AddMessage(t2)
        out_table = "Volumes_" + in_zone_data
        ...
        csv = out_table + ".csv"
        output_raster = gdb + "/"+ in_value_raster + "_ADJUSTED"
        ...
        raster_mask = ExtractByMask (in_value_raster, in_zone_data)
        ...
        arcpy.sa.ZonalStatisticsAsTable(in_zone_data, zone_field, adj_raster, out_table, 'DATA' , 'ALL')
        arcpy.AddMessage("Appending Feature...")
        Layer_Target_REM = arcpy.mapping.Layer(in_value_raster + "_ADJUSTED")
        arcpy.mapping.AddLayer(df, Layer_Target_REM)

        #Overwrite existing Volumes
        arcpy.DeleteField_management(in_zone_data, ["MEAN", "STD", "SUM", "COUNT"])
